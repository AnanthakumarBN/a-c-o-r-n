...
public class DrawingBean {
    ...
    public String createEmptyFile() throws DotFileException {
        ...
        if ((new File(filePath)).exists() && !(new File(filePath)).delete()) {
            throw new DotFileException("Can't delete file old DOT file: " + filePath + " . May be in use.");
        }
        try {
    ...
    /* opens graphic file generated by runNeato function
     * 
     */
    public void openGraphFile(String filePath) throws DotFileException {
        Image image = null;
        try {
            for (int i = 0; i <= 10; i++) {
